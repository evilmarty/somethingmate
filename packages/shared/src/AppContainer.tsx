...
const EMOJIS = ["❤️", "🍺", "🌯", "🥃", "🍦"];
const LINKS_TITLE = "You may also like...";

const AboutSection: FC<Partial<AppContainerProps>> = ({ about, links }) => (
  <div className="w-full navbar z-1">
    <div className="flex-1"></div>
    <div className="flex-none">
      <details className="dropdown dropdown-end dropdown-hover">
        <summary className="btn btn-square btn-ghost">
          <CircleQuestionMark className="inline-block h-5 w-5 stroke-current" />
        </summary>
        <div className="card dropdown-content bg-base-300 rounded-box z-1 w-60 p-2 shadow-sm">
          {about && (
          ...
          {links && (
            <ul className="menu w-full">
              <li className="menu-title">{LINKS_TITLE}</li>
...
const Container: FC<Partial<AppContainerProps>> = ({
  logo,
  name,
  children,
}) => (
...
const AppContainer: FC<AppContainerProps> = ({
  ...
  <div className="w-screen min-h-screen">
    {(about || links) && <AboutSection about={about} links={links} />}
    <Container logo={logo} name={name}>
      {children}
    </Container>
  </div>
);
